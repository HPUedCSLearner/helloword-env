...
void subtract_value_avx_64_f(std::vector<float>& arr, float value_to_subtract) {
    size_t array_size = arr.size();
    size_t i = 0;
...
int main() {
    ...
    // 计时循环展开的减法操作
    double time_subtract_value_sse_32_f = measure_time(subtract_value_sse_32_f, arr2, value_to_subtract);
    std::cout << "Unrolled loop + sse_32_f time: " << time_subtract_value_sse_32_f << " seconds" << std::endl;

    // 计时循环展开的减法操作
    double time_subtract_value_avx_64_f = measure_time(subtract_value_avx_64_f, arr2, value_to_subtract);
    std::cout << "Unrolled loop + avx_64_f time: " << time_subtract_value_avx_64_f << " seconds" << std::endl;
    ...
    // 计算加速比
    speedup = time_normal / time_subtract_value_sse_32_f;
    std::cout << "Unroll 32 + SSE Speedup: " << speedup << std::endl;

        // 计算加速比
    speedup = time_normal / time_subtract_value_avx_64_f;
    std::cout << "Unroll 64 + avx_64_f Speedup: " << speedup << std::endl;
